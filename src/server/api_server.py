...
def start_llama_cpp_server():
    ...
    cmd = f"./llama.cpp/build/bin/llama-server -m ./models/calm3-22b-chat-{quantize}.gguf -c 1024 -n 1024 -ngl 99 --host localhost --port 8080"
...
def delete_gguf():
    quantize = request.json['quantize']
    file_path = f"./models/calm3-22b-chat-{quantize}.gguf"
    try:
        os.remove(file_path)
